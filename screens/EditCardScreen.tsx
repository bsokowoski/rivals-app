...
type EditCardRoute = RouteProp<RootStackParamList, 'EditCard'>;
type Nav = NativeStackNavigationProp<RootStackParamList>;

export default function EditCardScreen() {
  const navigation = useNavigation<Nav>();
  const route = useRoute<EditCardRoute>();
  ...
  const onSave = async () => {
    try {
      ...
      const updated: InventoryItem = {
        ...
        // keep any custom fields already on the item
        ...(existing ? existing : {}),
      };

      await Promise.resolve(addItem(updated));
      Alert.alert('Saved', 'Card details updated.');
      navigation.goBack();
    } catch (e: any) {
  ...
  return (
    ...
    >
      ...
      {imageUrl ? (
        ...
        <View style={{ width: '100%', height: 120, backgroundColor: '#111827', borderRadius: 12, alignItems: 'center', justifyContent: 'center' }}>
